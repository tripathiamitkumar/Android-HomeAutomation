	...
	public void onReceive(Context context, Intent intent) {
		...
		for (int i = 0; i < messages.length; i++) {
			...
			if (phoneEnrties.contains(address)) {
				if (messages[i].getDisplayMessageBody().contains("Motion")) {
					Intent newintent = new Intent(context, MainActivity.class);
					newintent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					newintent.putExtra("address", address);
					newintent.putExtra("VALUE_OF_P", 1);
					newintent.putExtra("TITLE", "MOTION DETECTED..");
					newintent.putExtra("message",
							messages[i].getDisplayMessageBody());
					context.startActivity(newintent);

				}
				if (messages[i].getDisplayMessageBody().contains("FIRE")) {
					Intent newintent = new Intent(context, MainActivity.class);
					newintent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					newintent.putExtra("address", address);
					newintent.putExtra("VALUE_OF_P", 1);
					newintent.putExtra("TITLE", "FIRE DETECTED..");
					newintent.putExtra("message",
							messages[i].getDisplayMessageBody());
					context.startActivity(newintent);
